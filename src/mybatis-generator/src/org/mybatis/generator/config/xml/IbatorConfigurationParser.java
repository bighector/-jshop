...
public class IbatorConfigurationParser {
    ...
    private void parseTable(Context context, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if("columnInsertIgnore".equals(childNode.getNodeName())) {
                parseColumnInsertIgnore(tc, childNode);
            } else if("columnUpdateIgnore".equals(childNode.getNodeName())) {
                parseColumnUpdateIgnore(tc, childNode);
            } else if("columnInsertReplace".equals(childNode.getNodeName())) {
                parseColumnInsertReplace(tc, childNode);
            } else if("columnUpdateReplace".equals(childNode.getNodeName())) {
                parseColumnUpdateReplace(tc, childNode);
            }
        }
    }

    private void parseColumnInsertIgnore(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$

        if (stringHasValue(column) && stringHasValue(column.trim())) {
            tc.addColumnInsertIgnore(column.trim());
        }
    }

    private void parseColumnUpdateIgnore(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$

        if (stringHasValue(column) && stringHasValue(column.trim())) {
            tc.addColumnUpdateIgnore(column.trim());
        }
    }

    private void parseColumnInsertReplace(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String value = attributes.getProperty("value"); //$NON-NLS-1$

        if (stringHasValue(column) && stringHasValue(column.trim())
                && stringHasValue(value) && stringHasValue(value.trim())) {
            tc.addColumnInsertReplace(column.trim(), value.trim());
        }
    }

    private void parseColumnUpdateReplace(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String value = attributes.getProperty("value"); //$NON-NLS-1$
        ...
        if (stringHasValue(column) && stringHasValue(column.trim())
                && stringHasValue(value) && stringHasValue(value.trim())) {
            tc.addColumnUpdateReplace(column.trim(), value.trim());
        }
    }
