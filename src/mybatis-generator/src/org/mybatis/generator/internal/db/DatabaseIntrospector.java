...
public class DatabaseIntrospector {
    ...
    public List<IntrospectedTable> introspectTables(TableConfiguration tc)
        ...
        applyInsertIgnoreColumns(tc, columns);
        applyUpdateIgnoreColumns(tc, columns);
        applyInsertReplaceColumns(tc, columns);
        applyUpdateReplaceColumns(tc, columns);
    ...
    //标记列insert ignore
    private void applyInsertIgnoreColumns(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
            ...
            while (tableColumns.hasNext()) {
                ...
                if (tc
                        .isColumnInsertIgnored(introspectedColumn
                                .getActualColumnName())) {
                    introspectedColumn.setIsInsertIgnore(true);
                }
            }
        }
    }
    //标记列update ignore
    private void applyUpdateIgnoreColumns(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
                .entrySet()) {
            Iterator<IntrospectedColumn> tableColumns = (entry.getValue())
                    .iterator();
            while (tableColumns.hasNext()) {
                IntrospectedColumn introspectedColumn = tableColumns.next();
                if (tc
                        .isColumnUpdateIgnored(introspectedColumn
                                .getActualColumnName())) {
                    introspectedColumn.setIsUpdateIgnore(true);
                }
            }
        }
    }
    ...
    //标记列insert replace
    private void applyInsertReplaceColumns(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
                .entrySet()) {
            Iterator<IntrospectedColumn> tableColumns = (entry.getValue())
                    .iterator();
            while (tableColumns.hasNext()) {
                IntrospectedColumn introspectedColumn = tableColumns.next();
                if (tc.getColumnInsertReplaces().containsKey(introspectedColumn
                        .getActualColumnName().toLowerCase())) {
                    introspectedColumn.setInsertReplaceStr(tc.getColumnInsertReplaces().get(introspectedColumn
                            .getActualColumnName().toLowerCase()));
                }
            }
        }
    ...
    //标记列update replace
    private void applyUpdateReplaceColumns(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
                .entrySet()) {
            Iterator<IntrospectedColumn> tableColumns = (entry.getValue())
                    .iterator();
            while (tableColumns.hasNext()) {
                IntrospectedColumn introspectedColumn = tableColumns.next();
                if (tc.getColumnUpdateReplaces().containsKey(introspectedColumn
                        .getActualColumnName().toLowerCase())) {
                    introspectedColumn.setUpdateReplaceStr(tc.getColumnUpdateReplaces().get(introspectedColumn
                            .getActualColumnName().toLowerCase()));
                }
            }
        }
